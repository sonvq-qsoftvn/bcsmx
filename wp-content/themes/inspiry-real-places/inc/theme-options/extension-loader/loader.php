...
if ( !function_exists( 'inspiry_register_custom_extension_loader' ) ) :
    function inspiry_register_custom_extension_loader( $ReduxFramework ) {
    ...
    add_action("redux/extensions/inspiry_options/before", 'inspiry_register_custom_extension_loader', 0);
endif;
...
if ( !function_exists( 'wbc_importer_description_text' ) ) {
    function wbc_importer_description_text( $description ) {
        $message =  '<p>'. esc_html__( 'Only use demo importer on new WordPress install.', 'inspiry' ) . ' ' .
            esc_html__( 'Imported images are only for demo purpose.', 'inspiry' ) . ' ' .
            esc_html__( 'Import process can take few minutes to complete so please be patient and wait for its completion.', 'inspiry' ) .'</p>';
        return $message;
    }
...
if ( !function_exists( 'inspiry_after_import_settings' ) ) {
    function inspiry_after_import_settings( $demo_active_import , $demo_directory_path ) {
         ...
         * Setting Menu
         */
        $wbc_menu_array = array( 'default-demo', 'fallback-demo' );
